% ---------------------------------------------------------
% Gamma evaluation 2D: simple algorithm, local normalization
% ---------------------------------------------------------
% P Dvorak 2018
% ---------------------------------------------------------
...
% No interpolation. Local normalization of dose difference.
% ---------------------------------------------------------
%
% INPUT:
...
% ---------------------------------------------------------
%
% Syntax: myGammaLocalSimple(doseEvaluated,doseReference,dDose,dDistance,Treshold)
% ---------------------------------------------------------
%
...
function [pass_rate gammaMatrix] = myGammaLocal(doseEvaluated,doseReference,dDose,dDistance,Treshold)
% timeStart=tic;
...
for i=1:size(doseEvaluated,1)
    for j=1:size(doseEvaluated,2)
        ...
        if doseEvaluated(i,j)>=Treshold

            currentGamma=sqrt((doseEvaluated(i,j)-doseReference(i,j))^2/(dDose/100*doseEvaluated(i,j))^2);
         
%             k_radius=floor(currentGamma+1);
            k_radius=dDistance;
            
			k=1;

            while (k<=k_radius)%&&(k<=10)
                for kki=-k:k
                    for kkj=-k:k

                        if ((abs(kki)==k) || (abs(kkj)==k)) && (i+kki)>0 && (j+kkj)>0 && (i+kki)<size(doseReference,1) && (j+kkj)<size(doseReference,2) 
							newGamma=sqrt((doseEvaluated(i,j)-doseReference(i+kki,j+kkj))^2/(dDose/100*doseEvaluated(i,j))^2+(kki^2+kkj^2)/dDistance^2);    
...
% disp(['Fail rate =  ',num2str(length(N_fail)),' of total evaluated ',num2str(totalEvaluated),' points: Pass rate [%] = ',num2str(100-length(N_fail)/(totalEvaluated)*100)])
