...
decayEveryEpochs = 50;   % Decay every N epochs
...
% ---- Gamma Evaluation (Traceable Version) ----
[RefGammaVal, RefGammaMap] = myGamma_traceable(realImg, input, 2, 2, 10, 'local');
[PredGammaVal, PredGammaMap] = myGamma_traceable(genImg, input, 2, 2, 10, 'local');

% Smooth difference (no hard masking)
gammaDiff = abs(RefGammaMap - PredGammaMap);

GammaLoss = ( ...
    mean(abs(RefGammaVal - PredGammaVal),'all') + ...
    mean(gammaDiff,'all') ...
    ) * 10;

% ---- Standard pix2pixHD Losses ----
...
for i = 1:numFtrsMaps
    featureMatchingLoss = featureMatchingLoss + ...
    mean(abs(realPredFtrs{i} - genPredFtrs{i}), "all");
end
end
...
%%
function [pass_rate, gammaMap] = myGamma_traceable(doseEval, doseRef, dDose, dDistance, Threshold, mode)
   
    % Normalize to [0,100]
    doseE = ((doseEval + 1) / 2) * 100;
    doseR = ((doseRef + 1) / 2) * 100;
    
    % Max across spatial dimensions (keeping batch)
    maxRef = max(doseR, [], [1, 2]); 
    doseE = (doseE * 100) ./ (maxRef + eps);
    doseR = (doseR * 100) ./ (maxRef + eps);

    [H, W, ~, B] = size(doseE);
    
    % Threshold mask (Differentiable-friendly)
    evalMask = cast(doseE >= Threshold, 'like', doseE);
    
    % Neighbor shifts
    [dx, dy] = meshgrid(-dDistance:dDistance, -dDistance:dDistance);
    distSq = dx.^2 + dy.^2;
    validSearch = distSq <= dDistance^2 & distSq > 0;
    dx = dx(validSearch);
    dy = dy(validSearch);
    
    % Initialize with a large value for the min() operation
    minGammaSq = ones(size(doseE), 'like', doseE) * 1e6;

    % Loop over shifts (this is traceable, though heavy)
    for i = 1:numel(dx)
        shiftedRef = circshift(doseR, [dy(i), dx(i), 0, 0]);
        
        if strcmpi(mode, 'global')
            denom = (dDose/100 * 100)^2 + eps; % 100 because we normalized maxRef to 100
        else
            denom = (dDose/100 * doseE).^2 + eps;
        end
        
        doseTerm = ((doseE - shiftedRef).^2) ./ denom;
        spatialTerm = distSq(validSearch == 1); % Pick the current distance
        spatialTerm = spatialTerm(i) / (dDistance^2);
        
        totalTerm = doseTerm + spatialTerm;
        minGammaSq = min(minGammaSq, totalTerm);
    end

    gammaMap = sqrt(minGammaSq) .* evalMask;
    
    % Compute Pass Rate (Approximate for backprop)
    % Note: hard thresholds (>1) have zero gradient. 
    % For the loss function, use the gamma values directly.
    failCount = sum(cast(gammaMap > 1, 'like', gammaMap), [1, 2]);
    totalPixels = sum(evalMask, [1, 2]) + eps;
    pass_rate = 100 * (1 - (failCount ./ totalPixels));
end
